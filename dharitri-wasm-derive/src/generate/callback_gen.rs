...
pub fn generate_callback_selector_and_main(
    ...
    if let Some(raw) = raw_decl {
        ...
        if match_arms.is_empty() && module_calls.is_empty() {
            let cb_selector_body = quote! {
                dharitri_wasm::types::CallbackSelectorResult::NotProcessed(___cb_closure___)
            };
            let cb_main_body = quote! {};
            (cb_selector_body, cb_main_body)
        } else {
            ...
            let cb_main_body = quote! {
                if let Some(___cb_closure___) = dharitri_wasm::types::CallbackClosureForDeser::storage_load_and_clear::<Self::Api>() {
                    if let dharitri_wasm::types::CallbackSelectorResult::NotProcessed(_) =
...
fn callback_selector_body(
    ...
    quote! {
        ...
        if ___cb_closure_matcher___.matches_empty() {
            return dharitri_wasm::types::CallbackSelectorResult::Processed;
        }
        #(#match_arms)*
        #(#module_calls)*
        dharitri_wasm::types::CallbackSelectorResult::NotProcessed(___cb_closure___)
		...
		.map(|supertrait| {
			...
			quote! {
				match #module_path EndpointWrappers::callback_selector(self, ___cb_closure___) {
					...
					dharitri_wasm::types::CallbackSelectorResult::NotProcessed(recovered_cb_closure) => {
