...
pub fn generate_function_selector_body(contract: &ContractTrait) -> proc_macro2::TokenStream {
    let match_arms: Vec<proc_macro2::TokenStream> = contract
        ...
        .filter_map(|m| match &m.public_role {
            PublicRole::Init(_) => Some(endpoint_match_arm(
                m,
                "init",
                quote!{ if !<Self::Api as dharitri_wasm::api::VMApi>::external_view_init_override() },
            )),
    ...
    quote! {
        if match fn_name {
            ...
            b"init" if <Self::Api as dharitri_wasm::api::VMApi>::external_view_init_override() => {
                dharitri_wasm::external_view_contract::external_view_contract_constructor::<Self::Api>();
                return true;
            },
            #(#match_arms)*
            other => false
