...
use dharitri_sc_scenario::{managed_address, managed_biguint, managed_buffer, rust_biguint};
...
fn price_agg_submit_test() {
    let mut pa_setup = PriceAggSetup::new(dharitri_price_aggregator_sc::contract_obj);
    let current_timestamp = 100;
    let oracles = pa_setup.oracles.clone();

    // configure the number of decimals
    pa_setup.set_pair_decimals(MOAX_TICKER, USD_TICKER, DECIMALS);
