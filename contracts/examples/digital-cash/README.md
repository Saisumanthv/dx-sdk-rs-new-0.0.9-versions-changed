...
In order to fund one should first call `deposit_fees` depositing the fee funds as `MOAX` within the contract. Only after, if the fees cover the transfer of the certain number of tokens, it is possible to deposit the funds, making them available for claiming or forwarding.
