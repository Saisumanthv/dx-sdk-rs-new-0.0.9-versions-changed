...
deploy_ping_pong() {
    local FIXED_SUM=1500000000000000000 # 1.5 MOAX
    local DURATION=$1
    ...
    local RESULT_ADDRESS=$(moapy data parse --file="$OUTFILE" --expression="data['emitted_tx']['address']")
    local RESULT_TRANSACTION=$(moapy data parse --file="$OUTFILE" --expression="data['emitted_tx']['hash']")
