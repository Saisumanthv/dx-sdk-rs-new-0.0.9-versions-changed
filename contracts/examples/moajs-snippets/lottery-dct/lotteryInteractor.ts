 ...
 * Note: in production code, make sure you DO NOT reference the package "moajs-snippets".
 * Note: in dApps, make sure you use a proper wallet provider to sign the transaction.
 * @module
 */
import path from "path";
import { CodeMetadata, EnumValue, IAddress, Interaction, ResultsParser, ReturnCode, SmartContract, SmartContractAbi, Struct, TokenPayment, TransactionWatcher, VariadicValue } from "@dharitrinetwork/moajs";
import { INetworkProvider, ITestSession, ITestUser, loadAbiRegistry, loadCode } from "@dharitrinetwork/moajs-snippets";
import { NetworkConfig } from "@dharitrinetwork/moajs-network-providers";

const PathToWasm = path.resolve(__dirname, "..", "..", "lottery-dct", "output", "lottery-dct.wasm");
const PathToAbi = path.resolve(__dirname, "..", "..", "lottery-dct", "output", "lottery-dct.abi.json");
