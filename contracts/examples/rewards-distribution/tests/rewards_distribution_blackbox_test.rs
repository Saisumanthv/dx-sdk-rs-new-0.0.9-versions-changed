...
use dharitri_sc_scenario::{
    ...
    scenario_model::{
        ...
        SetStateStep, TxDCT, TypedResponse,
    },
            ...
            ScDeployStep::new()
                ...
                .call(
                    self.seed_nft_minter_contract
                        .init(TokenIdentifier::from_dct_bytes(NFT_TOKEN_ID)),
                ),
        );
...
fn test_raffle_and_claim() {
    ...
    let nft_payments: Vec<TxDCT> = nft_nonces
        .iter()
        .map(|nonce| TxDCT {
    ...
    for nonce in nft_nonces.iter() {
        alice_account =
            alice_account.dct_nft_balance(NFT_TOKEN_ID_EXPR, *nonce, "1", Option::<&[u8]>::None);
    }
    ...
    state.world.sc_call(
        ScCallStep::new()
            .from(ALICE_ADDRESS_EXPR)
            .multi_dct_transfer(nft_payments.clone())
            .call(
                state
                    .rewards_distribution_contract
    ...
    state.world.sc_call(
        ScCallStep::new()
            .from(ALICE_ADDRESS_EXPR)
            .multi_dct_transfer(nft_payments)
            .call(
                state
                    .rewards_distribution_contract
                    .claim_rewards(0u64, 0u64, reward_tokens),
