...
import { Account, Argument, BackendSigner, Balance, Code, ContractFunction, GasLimit, NetworkConfig, ProxyProvider, SmartContract } from "@dharitrinetwork/moajs";
import { AbiRegistry, U32Value } from "@dharitrinetwork/moajs/out/smartcontracts/typesystem";
import { BinaryCodec } from "@dharitrinetwork/moajs/out/smartcontracts/codec"
...
async function main() {
    ...
    let namespace = abi.findNamespace("lottery-moax");

    await NetworkConfig.getDefault().sync(provider);
    await user.sync(provider);

    // Deploy TRANSACTION
    let contractFile = path.resolve(__dirname, "../output/lottery-moax.wasm");
    ...
    let transactionStart = contract.call({
        ...
        args: [
            ...
            Argument.fromBigInt(Balance.MOAX(1).valueOf()),
