...
// Deposit 10 MOAX from alice to the multisig contract, which we'll use them in the next examples
await multisig.gas(20_000_000).sender(alice).value(Moax(10)).call.deposit();

// Create a proposal to send 3 MOAX from the multisig to eve
// Note that any proposal is automatically signed by alice
var sendId = await multisig.sender(alice).call.proposeSendMoax(eve, Moax(3));
...
var deployId = await multisig.sender(alice).gas(200_000_000).call.proposeSCDeploy(Moax(0), await adder.getCode(), false, false, false, formattedDeploy);
...
var addId = await multisig.sender(alice).call.proposeSendDct(adder, Moax(0), formattedAdd);
