...
use dharitri_wasm::types::{BoxedBytes, CodeMetadata, ManagedAddress};
...
use crate::multisig_rust_test_setup::{ActionRaw, MOAX_TOKEN_ID};
...
fn transfer_execute_to_user_test() {
    ...
    ms_setup
        .call_deposit(MOAX_TOKEN_ID, moax_amount)
        .assert_ok();

    ms_setup.b_mock.check_moax_balance(
        ms_setup.ms_wrapper.address_ref(),
        &rust_biguint!(moax_amount),
    );
    ...
    ms_setup
        .b_mock
        .check_moax_balance(&user_addr, &rust_biguint!(moax_amount));
}
