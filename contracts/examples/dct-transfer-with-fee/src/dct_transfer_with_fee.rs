...
pub trait DctTransferWithFee {
    ...
    fn set_exact_value_fee(
        ...
        self.token_fee(&token)
            .set(Fee::ExactValue(DctTokenPayment::new(
    ...
    fn claim_fees(&self) {
        ...
        for ((token, nonce), amount) in self.paid_fees().iter() {
            fees.push(DctTokenPayment::new(token, nonce, amount))
        }
    ...
    fn calculate_fee(
        &self,
        fee: &Fee<Self::Api>,
        mut provided: DctTokenPayment<Self::Api>,
    ) -> DctTokenPayment<Self::Api> {
        match fee {
