...
use dharitri_wasm::types::{
    BigInt, DctLocalRole, DctTokenPayment, DctTokenType, ManagedBuffer, ManagedVec, SCResult,
};
use dharitri_wasm_debug::{
    assert_sc_error, assert_values_eq, managed_address, managed_biguint, managed_buffer,
    managed_token_id, rust_biguint, testing_framework::*, tx_mock::TxInputDCT, unwrap_or_panic,
    DebugApi,
};
...
fn test_back_and_forth_transfers() {
    ...
    wrapper.execute_dct_multi_transfer(&user, &forwarder_wrapper, &transfers, |sc| {
        let mut managed_payments = ManagedVec::new();
        managed_payments.push(DctTokenPayment {
