...
pub trait ForwarderRaw {
    ...
    fn forward_payment(&self, to: ManagedAddress) {
        ...
        if token.is_moax() {
            self.send().direct_moax(&to, &payment, ManagedBuffer::new());
        } else {
    ...
    fn forward_contract_call(
        ...
        self.send()
            ...
            .with_moax_or_single_dct_token_transfer(payment_token, 0, payment_amount)
    ...
    fn callback_payments(&self) -> VecMapper<(MoaxOrDctTokenIdentifier, u64, BigUint)>;
