use dharitri_wasm::{
    api::BlockchainApi,
    types::{
        Address, BigUint, Box, DctTokenData, DctTokenType, ManagedAddress, ManagedBuffer,
...
impl BlockchainApi for crate::ArwenApiImpl {
    ...
    #[inline]
    #[cfg(not(feature = "unmanaged-ei"))]
    fn get_state_root_hash_managed(&self) -> dharitri_wasm::types::ManagedByteArray<Self, 32> {
        unsafe {
            let result_handle = mBufferNew();
            managedGetStateRootHash(result_handle);
            dharitri_wasm::types::ManagedByteArray::from_raw_handle(self.clone(), result_handle)
        }
    }
    ...
    #[inline]
    #[cfg(not(feature = "unmanaged-ei"))]
    fn get_tx_hash_managed(&self) -> dharitri_wasm::types::ManagedByteArray<Self, 32> {
        unsafe {
            let result_handle = mBufferNew();
    ...
    #[inline]
    #[cfg(not(feature = "unmanaged-ei"))]
    fn get_block_random_seed_managed(&self) -> dharitri_wasm::types::ManagedByteArray<Self, 48> {
        unsafe {
            let result_handle = mBufferNew();
    ...
    #[inline]
    #[cfg(not(feature = "unmanaged-ei"))]
    fn get_prev_block_random_seed_managed(&self) -> dharitri_wasm::types::ManagedByteArray<Self, 48> {
        unsafe {
            let result_handle = mBufferNew();
    ...
    #[inline]
    fn get_current_dct_nft_nonce(&self, address: &Address, token: &TokenIdentifier<Self>) -> u64 {
        unsafe {
            getCurrentDCTNFTNonce(
                address.as_ref().as_ptr(),
                token.to_dct_identifier().as_ptr(),
                token.len() as i32,
            ) as u64
        }
    }
