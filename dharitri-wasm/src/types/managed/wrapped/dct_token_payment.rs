...
use dharitri_codec::{
    dharitri_codec_derive::{NestedEncode, TopEncode},
...
impl<M: ManagedTypeApi> NestedDecode for DctTokenPayment<M> {
    #[cfg(not(feature = "dct-token-payment-legacy-decode"))]
    ...
    #[cfg(feature = "dct-token-payment-legacy-decode")]
    fn dep_decode_or_handle_err<I, H>(input: &mut I, h: H) -> Result<Self, H::HandledErr>
    where
        I: dharitri_codec::NestedDecodeInput,
        H: dharitri_codec::DecodeErrorHandler,
...
impl<M: ManagedTypeApi> IntoMultiValue for DctTokenPayment<M> {
    type MultiValue = DctTokenPaymentMultiValue<M>;
