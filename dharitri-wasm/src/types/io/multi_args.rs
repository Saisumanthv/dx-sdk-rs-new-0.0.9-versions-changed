...
macro_rules! multi_arg_impls {
    ($(($mval_struct:ident $marg_struct:ident $mres_struct:ident $($n:tt $name:ident)+) )+) => {
        $(
            ...
            pub type $marg_struct<$($name,)+> = dharitri_codec::multi_types::$mval_struct<$($name,)+>;

            #[deprecated(
                since = "0.9.2",
                note = "Alias kept for backwards compatibility. Replace with `MultiValue*`"
            )]
            pub type $mres_struct<$($name,)+> = dharitri_codec::multi_types::$mval_struct<$($name,)+>;

            impl<$($name),+ > TypeAbi for dharitri_codec::multi_types::$mval_struct<$($name,)+>
            where
                $($name: TypeAbi,)+
            {
                fn type_name() -> String {
