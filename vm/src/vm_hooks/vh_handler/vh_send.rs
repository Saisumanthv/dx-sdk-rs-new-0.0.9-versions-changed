use crate::{
    tx_execution::builtin_function_names::{
        DCT_MULTI_TRANSFER_FUNC_NAME, DCT_NFT_TRANSFER_FUNC_NAME, DCT_TRANSFER_FUNC_NAME,
...
pub trait VMHooksSend: VMHooksHandlerSource {
    fn perform_transfer_execute_dct(
        &self,
        to: VMAddress,
        token: Vec<u8>,
    ...
    fn perform_transfer_execute_nft(
        ...
        self.perform_transfer_execute(
            contract_address,
            num_bigint::BigUint::zero(),
            DCT_NFT_TRANSFER_FUNC_NAME.into(),
            args,
        );
    }
    ...
    fn perform_upgrade_contract(
        ...
        self.perform_async_call(to, moax_value, UPGRADE_CONTRACT_FUNC_NAME.into(), arguments)
    }
    ...
    fn transfer_value_execute(
        ...
        let moax_value = self.m_types_borrow().bu_get(amount_handle);
        ...
        self.perform_transfer_execute(recipient, moax_value, endpoint_name, arg_buffer);
    ...
    fn multi_transfer_dct_nft_execute(
        ...
        let payments = self
            .m_types_borrow()
            .mb_get_vec_of_dct_payments(payments_handle);
        let endpoint_name = self
            .m_types_borrow()
            .mb_to_function_name(endpoint_name_handle);
        let arg_buffer = self.m_types_borrow().mb_get_vec_of_bytes(arg_buffer_handle);
        ...
        if payments.len() == 1 {
            ...
            if payment.nonce == 0 {
                self.perform_transfer_execute_dct(
                    to,
    ...
    fn async_call_raw(
        &self,
        to_handle: RawHandle,
        moax_value_handle: RawHandle,
        endpoint_name_handle: RawHandle,
        arg_buffer_handle: RawHandle,
    ) -> ! {
        let to = self.m_types_borrow().mb_to_address(to_handle);
        let moax_value = self.m_types_borrow().bu_get(moax_value_handle);
        let endpoint_name = self
            .m_types_borrow()
            .mb_to_function_name(endpoint_name_handle);
        let arg_buffer = self.m_types_borrow().mb_get_vec_of_bytes(arg_buffer_handle);

        self.perform_async_call(to, moax_value, endpoint_name, arg_buffer)
    }
    ...
    fn deploy_contract(
        ...
        let (new_address, result) =
            self.perform_deploy(moax_value, code, code_metadata, arg_buffer);
    ...
    fn deploy_from_source_contract(
        ...
        let (new_address, result) =
            self.perform_deploy(moax_value, source_contract_code, code_metadata, arg_buffer);
    ...
    fn upgrade_contract(
        ...
        self.perform_upgrade_contract(to, moax_value, code, code_metadata, arg_buffer)
    }
    ...
    fn execute_on_dest_context_raw(
        ...
        let result =
            self.perform_execute_on_dest_context(to, moax_value, endpoint_name, arg_buffer);
