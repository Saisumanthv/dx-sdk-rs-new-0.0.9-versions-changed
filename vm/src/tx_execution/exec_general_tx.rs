...
fn should_add_transfer_value_log(tx_input: &TxInput) -> bool {
    if tx_input.call_type == CallType::AsyncCallback
        && !tx_input.callback_payments.dct_values.is_empty()
    {
        // edge case in the VM
        return false;
    }
    ...
    tx_input.from.is_smart_contract_address() && !tx_input.moax_value.is_zero()
}
...
pub(crate) fn create_transfer_value_log(tx_input: &TxInput, call_type: CallType) -> TxLog {
    ...
    let moax_value = if tx_input.call_type == CallType::AsyncCallback {
        &tx_input.callback_payments.moax_value
    } else {
        &tx_input.moax_value
    };
    ...
    TxLog {
        ...
        topics: vec![top_encode_big_uint(moax_value), tx_input.to.to_vec()],
        data,
    }
}
...
impl BlockchainVMRef {
    ...
    {
        if let Err(err) =
            tx_cache.transfer_moax_balance(&tx_input.from, &tx_input.to, &tx_input.moax_value)
        {
