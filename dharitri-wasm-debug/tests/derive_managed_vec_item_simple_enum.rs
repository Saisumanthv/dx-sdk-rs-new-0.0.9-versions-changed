...
fn enum_static() {
    assert_eq!(
        <SimpleEnum as dharitri_wasm::types::ManagedVecItem>::PAYLOAD_SIZE,
        1
    );
    assert!(<SimpleEnum as dharitri_wasm::types::ManagedVecItem>::SKIPS_RESERIALIZATION);
}

#[test]
fn enum_to_bytes_writer() {
    <SimpleEnum as dharitri_wasm::types::ManagedVecItem>::to_byte_writer(
        &SimpleEnum::Variant1,
        |bytes| {
...
fn enum_from_bytes_reader() {
    let enum_from_bytes =
        <SimpleEnum as dharitri_wasm::types::ManagedVecItem>::from_byte_reader(|bytes| {
            assert_eq!(bytes.len(), 1);
            bytes[0] = 1;
        });
