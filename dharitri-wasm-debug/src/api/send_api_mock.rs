...
use dharitri_wasm::{
    ...
    types::{
        BigUint, BoxedBytes, CodeMetadata, DctTokenPayment, ManagedAddress, ManagedArgBuffer,
...
impl TxContext {
    fn get_available_moax_balance(&self) -> num_bigint::BigUint {
    ...
    fn get_available_dct_balance(&self, token_identifier: &[u8]) -> num_bigint::BigUint {
        // start with the pre-existing balance
        ...
        if self.tx_input_box.dct_token_identifier == token_identifier {
...
impl SendApi for TxContext {
    fn direct_moax<D>(&self, to: &ManagedAddress<Self>, amount: &BigUint<Self>, _data: D)
    where
        D: ManagedInto<Self, ManagedBuffer<Self>>,
    {
        let amount_value = self.big_uint_value(amount);
        if amount_value > self.get_available_moax_balance() {
    ...
    fn direct_dct_execute(
        ...
        let amount_value = self.big_uint_value(amount);
        if amount_value > self.get_available_dct_balance(token.to_dct_identifier().as_slice()) {
            std::panic::panic_any(TxPanic {
                status: 10,
        ...
        let recipient = to.to_address();
        let token_identifier = token.to_dct_identifier();
        let mut tx_output = self.tx_output_cell.borrow_mut();
        tx_output.send_balance_list.push(SendBalance {
            recipient,
            token_identifier,
    ...
    fn direct_multi_dct_transfer_execute(
        &self,
        _to: &ManagedAddress<Self>,
        _payments: &ManagedVec<Self, DctTokenPayment<Self>>,
        _gas_limit: u64,
        _endpoint_name: &ManagedBuffer<Self>,
        _arg_buffer: &ManagedArgBuffer<Self>,
    ) -> Result<(), &'static [u8]> {
        panic!("direct_multi_dct_transfer_execute not implemented yet");
    }
