...
pub fn execute_dct_multi_transfer(
    tx_input: TxInput,
    tx_cache: TxCache,
) -> (TxResult, BlockchainUpdate) {
    if tx_input.args.len() < 2 {
        let err_result =
            TxResult::from_vm_error("MultiDCTNFTTransfer too few arguments".to_string());
        return (err_result, BlockchainUpdate::empty());
    }
    assert!(
        tx_input.to == tx_input.from,
        "MultiDCTNFTTransfer expects that to == from"
    );
    ...
    for _ in 0..payments {
        ...
        builtin_logs.push(TxLog {
            ...
            endpoint: DCT_MULTI_TRANSFER_FUNC_NAME.to_vec(),
            topics: vec![
                token_identifier,
                nonce_bytes,
                value_bytes,
