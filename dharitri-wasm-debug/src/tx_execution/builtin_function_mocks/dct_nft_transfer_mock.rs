...
pub fn execute_dct_nft_transfer(
    tx_input: TxInput,
    tx_cache: TxCache,
) -> (TxResult, BlockchainUpdate) {
    if tx_input.args.len() < 4 {
        let err_result = TxResult::from_vm_error("DCTNFTTransfer too few arguments".to_string());
        return (err_result, BlockchainUpdate::empty());
    }
    assert!(
        tx_input.to == tx_input.from,
        "DCTNFTTransfer expects that to == from"
    );
    ...
    let dct_nft_transfer_log = TxLog {
        address: tx_input.from.clone(),
        endpoint: DCT_NFT_TRANSFER_FUNC_NAME.to_vec(),
        topics: vec![
