...
use crate::{denali_step, world_mock::BlockchainMock, ContractMap, DebugApi};
...
pub fn denali_rs<P: AsRef<Path>>(relative_path: P, contract_map: ContractMap<DebugApi>) {
    ...
    for step in scenario.steps.iter() {
        match step {
            ...
            Step::ValidatorReward { tx_id, comment, tx } => {
                Rc::get_mut(state)
                    .unwrap()
                    .increase_validator_reward(&tx.to.value.into(), &tx.moax_value.value);
            },
